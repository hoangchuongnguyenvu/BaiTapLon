...
def main():
    ...
    # Thêm xử lý lỗi cho file video
    def load_video(path):
        try:
            with open(path, 'rb') as video_file:
                return video_file.read()
        except FileNotFoundError:
            st.error(f"Không tìm thấy file video: {path}")
            return None
        except Exception as e:
            st.error(f"Lỗi khi tải video: {str(e)}")
            return None

    # Thay thế code tải video bằng phiên bản an toàn hơn
    with col1:
        st.markdown("**Video 1**")
        video_bytes = load_video('new_videos/output.mp4')
        if video_bytes:
            st.video(video_bytes)

    with col2:
        st.markdown("**Video 2**")
        video_bytes = load_video('new_videos/output1.mp4')
        if video_bytes:
            st.video(video_bytes)
